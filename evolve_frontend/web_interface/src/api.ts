...
export const createAgent = async (agentData: Record<string, any>): Promise<any> => {
  try {
    const response = await fetch(`${API_BASE_URL}/api/agents`, {
      method: 'POST',
...
export const runAgent = async (agentId: string, input: string): Promise<any> => {
  try {
...
export const getAgentStatus = async (agentId: string): Promise<{ status: string }> => {
  try {
...
export const getAgentResults = async (
  agentId: string,
  runId: string
): Promise<{ results: any[] }> => {
  try {
    const response = await fetch(`${API_BASE_URL}/api/agents/${agentId}/runs/${runId}/results`);
    if (!response.ok) throw new Error('Failed to get agent results');
    return await response.json();
  } catch (error) {
...
export const sendMessage = async (input: string): Promise<{ message: string }> => {
